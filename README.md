...
- ウォッチモードを追加
  - `npm run watch:svr` でファイル変更を自動検出してビルド
  - svrモードと組み合わせることで開発効率が向上
...
1. `npm run watch` でウォッチモードを起動
2. 初回のみ `npm run pack:svr` でプラグインをパッケージングしてkintoneにインストール
3. ローカルサーバー（Live Serverなど）を起動
4. 以降、ソースコードを編集すると自動でビルド・反映されます
